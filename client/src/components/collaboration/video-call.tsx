...
interface VideoCallProps {
  ...
  onClose: () => void;
  groupId?: string; // if provided, enable in-call group chat
}
    ...
    try {
      ...
      if (localVideoRef.current) {
        ...
        try {
          ...
          // Some browsers require user gesture; ignore and rely on autoplay attribute
        }
      }
      ...
      // Wire ICE candidates out via websocket
      webrtcManager.current.onIceCandidate((userId, candidate) => {
        socket.current.sendWebRTCIce(userId, candidate, { noteId });
      });

      // Listen for signaling
      const onOffer = async (msg: any) => {
        if (msg.type !== "webrtc_offer") return;
        const answer = await webrtcManager.current.createAnswer(
          msg.userId || msg.from || "peer",
          msg.offer
        );
        socket.current.sendWebRTCAnswer(
          msg.userId || msg.from || "peer",
          answer,
          { noteId }
        );
      ...
      const onAnswer = async (msg: any) => {
        if (msg.type !== "webrtc_answer") return;
        await webrtcManager.current.setRemoteAnswer(
          msg.userId || msg.from || "peer",
          msg.answer
        );
      };
      const onIce = async (msg: any) => {
        if (msg.type !== "webrtc_ice") return;
        await webrtcManager.current.addIceCandidate(
          msg.userId || msg.from || "peer",
          msg.candidate
        );
      };
      socket.current.on("webrtc_offer", onOffer);
      socket.current.on("webrtc_answer", onAnswer);
      socket.current.on("webrtc_ice", onIce);
          ...
          <div className="flex">
            <div
              className={`flex-1 p-4 ${
                showChat && groupId ? "border-r border-gray-800" : ""
              }`}
            >
              <div className="video-grid h-96">
                ...
                {participants.slice(0, 5).map((participant, index) => (
                  <div
                    key={participant.id}
                    className="relative bg-gray-800 rounded-lg overflow-hidden video-participant"
                  >
                    <div className="absolute inset-0 flex items-center justify-center bg-gray-700">
                      <Avatar className="w-16 h-16">
            ...
            {groupId && showChat && (
              <div className="w-full sm:w-96 max-w-full bg-white h-[calc(90vh-120px)] overflow-hidden">
