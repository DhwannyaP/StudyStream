...
export default function TeacherNotes() {
  ...
  const { data: notes, isLoading: notesLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/notes?teacherId=${teacherId}`, {
        headers: {
  ...
  const { data: teacher, isLoading: teacherLoading } = useQuery({
    ...
    select: (data) => (data as any[])?.find((t: any) => t.id === teacherId),
  });
  ...
  const onlineCount =
    (sessions as any[])?.filter((s: any) => s.isActive && s.noteId)?.length ||
    0;
        ...
        <Card className="mb-8">
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="font-heading text-2xl font-bold text-edu-text-primary">
                  {teacher.fullName}
                </h1>
        ...
        {(notes as any[])?.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="font-heading text-xl font-bold text-edu-text-primary mb-2">
                No Notes Available
              </h3>
              <p className="text-edu-text-secondary">
                This teacher hasn't uploaded any materials yet.
              </p>
            </CardContent>
          </Card>
        ) : (
              ...
              >
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div
                      className={`p-3 rounded-lg ${getFileTypeColor(
                        note.fileType
                      )}`}
                    >
                  ...
                  <div className="flex items-center justify-between text-xs text-edu-text-secondary">
                    ...
                    <span>
                      {note.fileType === "application"
                        ? "Document"
                        : note.fileType === "image"
                        ? "Images"
                        : `${Math.floor(Math.random() * 50)} pages`}
                    </span>
                  </div>
